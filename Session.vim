...
nnoremap <silent> <Plug>(go-metalinter) :call go#lint#Gometa('')
...
set nomodeline
set ruler
set runtimepath=~/.vim,~/.vim/bundle/vim-go,/var/lib/vim/addons,/usr/share/vim/vimfiles,/usr/share/vim/vim74,/usr/share/vim/vimfiles/after,/var/lib/vim/addons/after,~/.vim/after
set suffixes=.bak,~,.swp,.o,.info,.aux,.log,.dvi,.bbl,.blg,.brf,.cb,.ind,.idx,.ilg,.inx,.out,.toc
let s:so_save = &so | let s:siso_save = &siso | set so=0 siso=0
let v:this_session=expand("<sfile>:p")
silent only
cd ~/GOPATH/src/github.com/gersongraciani/ggping/shared
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +0 logic_test.go
badd +0 logic.go
badd +0 models.go
args logic_test.go
edit logic.go
...
exe 'vert 2resize ' . ((&columns * 52 + 84) / 168)
exe '3resize ' . ((&lines * 28 + 22) / 44)
exe 'vert 3resize ' . ((&columns * 115 + 84) / 168)
argglobal
